#!/usr/bin/env node
// signaltrim — unified cross-platform installer.
//
...
//   curl|bash:   delegated from install.sh shim → npx -y github:karurikwao/signaltrim -- [flags]
...
const REPO = 'karurikwao/signaltrim';
// Remote hook fetch ref. Launch builds default to main so GitHub installs work
// before the first SignalTrim release tag exists. Release automation can set a
// tag here after regenerating src/hooks/checksums.sha256 for that exact ref.
// Overridable via SIGNALTRIM_REF for testing against a branch or tag.
const PINNED_REF = process.env.SIGNALTRIM_REF || 'main';
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // through to the switch and is rejected — signaltrim-shrink is a proxy
    ...
    switch (a) {
      ...
      case '--with-mcp-shrink': {
        ...
        if (v && !v.startsWith('--')) {
          ...
          die('error: --with-mcp-shrink requires an upstream command — signaltrim-shrink\n' +
  ...
  //   • withMcpShrink — signaltrim-shrink is a proxy that needs an upstream
  ...
  if (opts.only.length) {
    ...
    for (const id of opts.only) {
      if (!knownIds.has(id)) {
        die(`error: unknown agent: ${id}\n  see 'signaltrim --list' for valid ids`);
      }
    }
  }
  return opts;
...
function checkWslWindowsNode() {
  ...
  if (process.env.WSL_DISTRO_NAME) {
    die('signaltrim: detected Windows Node.js running inside WSL.\n' +
  ...
  try {
    ...
    if (v.includes('microsoft') || v.includes('wsl')) {
      die('signaltrim: detected Windows Node.js running inside WSL (/proc/version).\n' +
          '         Install Linux-native Node inside your WSL distro and re-run there.');
    }
...
// Node's spawnSync('claude', ...) returns ENOENT for these unless cmd.exe
// handles PATHEXT. That requires a shell, so every shell-mediated argument is
// quoted and arguments containing cmd.exe metacharacters are refused.
const IS_WIN = process.platform === 'win32';
const WIN_CMD_META = /[&|<>^%!()\r\n]/;

function assertSafeWinShellToken(value, label) {
  const s = String(value);
  if (WIN_CMD_META.test(s)) {
    throw new Error(`Refusing unsafe Windows shell ${label}: contains cmd.exe metacharacter`);
  }
}
...
function quoteWinArg(a) {
  if (!IS_WIN) return a;
  const s = String(a);
  assertSafeWinShellToken(s, 'argument');
  if (s === '' || /[\s"]/.test(s)) {
...
function spawnXplat(cmd, args, opts) {
  if (IS_WIN) {
    try {
      assertSafeWinShellToken(cmd, 'command');
      const quoted = [quoteWinArg(cmd), ...args.map(quoteWinArg)].join(' ');
      return child_process.spawnSync(quoted, [], Object.assign({ shell: true }, opts || {}));
    } catch (err) {
      return {
        status: 1,
        error: err,
...
async function installClaude(ctx) {
  ...
  if (alreadyInstalled) {
    note('  signaltrim plugin already installed (use --force to reinstall)');
  ...
  // signaltrim-*.js scripts, so Claude Code crashes every SessionStart /
  ...
  // skip standalone wiring because the plugin manifest handles hooks.
  {
    ...
    if (settings) {
      ...
      if (pruned > 0) {
        note(`  removed ${pruned} orphaned signaltrim hook entr${pruned === 1 ? 'y' : 'ies'} from settings.json (target script missing)`);
        if (!opts.dryRun) {
  ...
  // event (two SIGNALTRIM MODE blocks, two reinforcement lines).
  let shouldWireHooks;
  ...
  if (opts.withMcpShrink) {
    say('  → wiring signaltrim-shrink MCP proxy (--with-mcp-shrink)');
    const r = installMcpShrink(ctx);
    if (r.kind === 'ok')   results.installed.push('signaltrim-shrink');
    if (r.kind === 'skip') results.skipped.push(['signaltrim-shrink', r.why]);
    if (r.kind === 'fail') results.failed.push(['signaltrim-shrink', r.why]);
  }
...
function installGemini(ctx) {
  ...
  if (!opts.force) {
    ...
    if (r.status === 0 && /signaltrim/i.test(r.stdout || '')) {
      note('  signaltrim extension already installed (use --force to reinstall)');
...
// Drops the signaltrim skills into ~/.hermes/skills/productivity/ (or HERMES_HOME if set).
const HERMES_SKILL_DIRS = ['signaltrim', 'signaltrim-commit', 'signaltrim-review', 'signaltrim-help', 'signaltrim-stats', 'signaltrim-compress', 'signalteam'];
...
function installHermes(ctx) {
  ...
  if (!repoRoot) {
    warn('  Hermes native install requires a local clone of the signaltrim repo.');
    note('  Re-run from a clone: git clone https://github.com/' + REPO + ' && cd signaltrim && node bin/install.js --only hermes');
...
const OPENCODE_SKILL_DIRS  = ['signaltrim', 'signaltrim-commit', 'signaltrim-review', 'signaltrim-help', 'signaltrim-stats', 'signaltrim-compress', 'signalteam'];
const OPENCODE_AGENT_FILES = ['signalteam-investigator.md', 'signalteam-builder.md', 'signalteam-reviewer.md'];
const OPENCODE_COMMAND_FILES = ['signaltrim.md', 'signaltrim-commit.md', 'signaltrim-review.md', 'signaltrim-compress.md', 'signaltrim-stats.md', 'signaltrim-help.md'];
const OPENCODE_PLUGIN_REL = './plugins/signaltrim/plugin.js';
const OPENCODE_AGENTS_MD_SENTINEL = 'SignalTrim compact mode active';
...
const OPENCODE_AGENTS_MD_BEGIN = '<!-- signaltrim-begin -->';
const OPENCODE_AGENTS_MD_END = '<!-- signaltrim-end -->';
...
function installOpencode(ctx) {
  ...
  if (!repoRoot) {
    warn('  opencode native install requires a local clone of the signaltrim repo.');
    note('  Re-run from a clone: git clone https://github.com/' + REPO + ' && cd signaltrim && node bin/install.js --only opencode');
  ...
  const pluginDir   = path.join(dir, 'plugins', 'signaltrim');
  ...
  if (opts.dryRun) {
    ...
    note(`  would copy plugin.js + package.json + signaltrim-config.cjs into ${pluginDir}/`);
    note(`  would copy ${OPENCODE_COMMAND_FILES.length} command files into ${commandsDir}/`);
    note(`  would copy ${OPENCODE_AGENT_FILES.length} signalteam agents into ${agentsDir}/`);
  ...
  try {
    // 1. Plugin dir — copy plugin.js, package.json, signaltrim-config.js (sibling).
    ...
    const pluginPayload = [
      ...
      [path.join(repoRoot, 'src', 'hooks', 'signaltrim-config.js'),
       path.join(pluginDir, 'signaltrim-config.cjs')],
    ];
    ...
    const ruleBody = fs.readFileSync(path.join(repoRoot, 'src', 'rules', 'signaltrim-activate.md'), 'utf8').trimEnd() + '\n';
    ...
    if (fs.existsSync(agentsMd)) {
      ...
      if (alreadyFenced) {
        note(`  ${agentsMd} already contains signaltrim ruleset`);
      } else if (alreadyByLegacySentinel) {
        if (!opts.force) {
          note(`  ${agentsMd} contains a legacy (un-fenced) signaltrim block — leaving as-is`);
        ...
        process.stdout.write(`  appended signaltrim ruleset to ${agentsMd}\n`);
      }
    } else {
    ...
    // 6. opencode.json — add plugin entry; optional signaltrim-shrink MCP.
    ...
    if (opts.withMcpShrink) {
      ...
      // produced. signaltrim-shrink is a proxy — it crashes without an upstream,
...
// Drops skills/signaltrim/ into the OpenClaw workspace and appends a small
...
// signaltrim discoverable via `openclaw skills list`. See bin/lib/openclaw.js
...
async function installHooks(ctx) {
  ...
  try { fs.chmodSync(path.join(hooksDir, 'signaltrim-statusline.sh'), 0o755); } catch (_) {}
  ...
  const activate = path.join(hooksDir, 'signaltrim-activate.js');
  const tracker  = path.join(hooksDir, 'signaltrim-mode-tracker.js');
  const statusline = path.join(hooksDir, 'signaltrim-statusline.sh');
  ...
  SETTINGS.addCommandHook(settings, 'UserPromptSubmit', {
    ...
    marker: 'signaltrim-mode-tracker',
    timeout: 5,
  ...
  const slCmd = IS_WIN
    ? `${psHost} -NoProfile -ExecutionPolicy Bypass -File "${path.join(hooksDir, 'signaltrim-statusline.ps1')}"`
    : `bash "${statusline}"`;
  if (!settings.statusLine) {
    ...
    if (existing.includes(statusline) || existing.includes('signaltrim-statusline')) {
      process.stdout.write('  statusline badge already configured.\n');
    } else {
      process.stdout.write('  NOTE: existing statusline detected — signaltrim badge NOT added.\n');
...
function installMcpShrink(ctx) {
  ...
  if (spawnOk(r)) {
    ...
    note(`    Edit ~/.claude.json mcpServers["signaltrim-shrink"] to change the upstream,`);
    note('    or `claude mcp remove signaltrim-shrink` to drop it.');
    note(`    Docs: https://github.com/${REPO}/tree/main/src/mcp-servers/signaltrim-shrink`);
    return { kind: 'ok' };
  }
...
async function runInit(ctx) {
  ...
  const local = repoRoot && path.join(repoRoot, 'src/tools/signaltrim-init.js');
  ...
  try {
    const tmp = path.join(os.tmpdir(), `signaltrim-init-${process.pid}.js`);
...
async function loadRemoteHookChecksums() {
  const tmp = path.join(os.tmpdir(), `signaltrim-checksums-${process.pid}-${Date.now()}.sha256`);
  try {
...
function uninstall(ctx) {
  ...
  if (fs.existsSync(settingsPath)) {
    const settings = SETTINGS.readSettings(settingsPath);
    if (settings) {
      const removed = SETTINGS.removeSignalTrimHooks(settings);
      ...
      if (settings.statusLine) {
        ...
        if (cmd.includes('signaltrim-statusline')) delete settings.statusLine;
      }
      SETTINGS.validateHookFields(settings);
      if (!opts.dryRun) SETTINGS.writeSettings(settingsPath, settings);
      ok(`  removed ${removed} signaltrim hook entr${removed === 1 ? 'y' : 'ies'} from settings.json`);
    }
  }
  ...
  // machine where signaltrim was never installed (or was already removed) doesn't
  // print "Plugin not installed" stderr noise.
  if (hasCmd('claude')) {
    const probe = captureSpawn('claude', ['plugin', 'list']);
    if (probe.status === 0 && /signaltrim/i.test(probe.stdout || '')) {
      const r = runSpawn('claude', ['plugin', 'uninstall', 'signaltrim@signaltrim'], null, opts.dryRun);
      if (spawnOk(r)) ok('  removed claude plugin');
    } else {
      note('  claude plugin not installed — skipping');
    }

    // signaltrim-shrink MCP — only run if `claude mcp` subcommand exists. Tolerate
    // non-zero exit (server may have never been registered).
    const mcpHelp = captureSpawn('claude', ['mcp', '--help']);
    if (mcpHelp.status === 0) {
      runSpawn('claude', ['mcp', 'remove', 'signaltrim-shrink'], null, opts.dryRun);
    }
  }
  ...
  if (hasCmd('gemini')) {
    ...
    if (probe.status === 0 && /signaltrim/i.test(probe.stdout || '')) {
      runSpawn('gemini', ['extensions', 'uninstall', 'signaltrim'], null, opts.dryRun);
    } else {
  ...
  const ocPluginDir = path.join(ocDir, 'plugins', 'signaltrim');
  if (fs.existsSync(ocPluginDir)) {
    ...
    if (fs.existsSync(ocJson)) {
      ...
      if (cfg) {
        ...
        if (cfg.mcp && typeof cfg.mcp === 'object' && cfg.mcp['signaltrim-shrink']) {
          delete cfg.mcp['signaltrim-shrink'];
          if (Object.keys(cfg.mcp).length === 0) delete cfg.mcp;
        }
        if (!opts.dryRun) SETTINGS.writeSettings(ocJson, cfg);
        ok(`  pruned signaltrim entries from ${ocJson}`);
      }
    }
    ...
    // AGENTS.md — strip the fenced signaltrim block (preserves user content
    ...
    if (fs.existsSync(ocAgentsMd)) {
      ...
      if (begin !== -1 && end !== -1 && end > begin) {
        ...
        note(next === '' ? `  removed ${ocAgentsMd}` : `  stripped signaltrim block from ${ocAgentsMd}`);
        ...
        if (body.trim() === '' || body.trim().startsWith(OPENCODE_AGENTS_MD_SENTINEL)) {
          ...
          note(`  left ${ocAgentsMd} in place (legacy mixed content — strip signaltrim block manually)`);
        }
      }
    }
    // opencode flag file
    const ocFlag = path.join(ocDir, '.signaltrim-active');
  ...
  if (fs.existsSync(path.join(ocwWs, 'skills', 'signaltrim')) || fs.existsSync(path.join(ocwWs, 'SOUL.md'))) {
    const log = {
      write: (s) => process.stdout.write(s),
      note: (s) => note(s),
      warn: (s) => warn(s),
    ...
    if (r.touched) ok('  pruned signaltrim entries from OpenClaw workspace');
  }
  ...
  if (fs.existsSync(hermesRoot)) {
    ...
    if (prunedHermes) ok('  pruned signaltrim skills from Hermes');
  }

  // Flag file
  const flag = path.join(configDir, '.signaltrim-active');
...
function printList(noColor) {
  ...
  process.stdout.write(c.orange('SignalTrim provider matrix') + '\n\n');
...
// ── Help ───────────────────────────────────────────────────────────────────
function printHelp() {
  process.stdout.write(`signaltrim installer — detects your agents and installs signaltrim for each one.

USAGE
  npx -y github:karurikwao/signaltrim -- [flags]
...
EXAMPLES
  npx -y github:karurikwao/signaltrim                        # default install
  npx -y github:karurikwao/signaltrim -- --all               # all the trimmings
  npx -y github:karurikwao/signaltrim -- --only claude --no-mcp-shrink
...
async function main() {
  ...
  if (opts.withInit) {
    ...
    if (await runInit(ctx)) ctx.results.installed.push(`signaltrim-init (${process.cwd()})`);
    else                    ctx.results.failed.push(['signaltrim-init', 'src/tools/signaltrim-init.js failed']);
    process.stdout.write('\n');
