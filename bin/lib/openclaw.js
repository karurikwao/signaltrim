// signaltrim → OpenClaw install / uninstall helper.
//
...
// and 60K-total cap.
//
// To make signaltrim always-on through OpenClaw, we do two writes:
//   1. Drop a copy of skills/signaltrim/SKILL.md into <workspace>/skills/signaltrim/
...
const SKILL_NAME = 'signaltrim';
const SKILL_VERSION = '1.0.0';
const MARK_BEGIN = '<!-- signaltrim-begin -->';
const MARK_END = '<!-- signaltrim-end -->';
...
// signaltrim SKILL.md uses block-scalar `description: >`, which a naive split
...
function loadBootstrapSnippet(repoRoot) {
  if (repoRoot) {
    const p = path.join(repoRoot, 'src', 'rules', 'signaltrim-openclaw-bootstrap.md');
    const body = readIfExists(p);
    if (body) return body.endsWith('\n') ? body : body + '\n';
  }
  // Standalone fallback (curl|node case where there's no repo on disk).
  // Keep this in sync with src/rules/signaltrim-openclaw-bootstrap.md.
  return [
    MARK_BEGIN,
    '## SignalTrim mode (always on)',
    '',
    'SignalTrim compact mode active. Preserve technical substance. Remove filler.',
    '',
    "The full ruleset and intensity levels live in this workspace's signaltrim skill:",
    '',
    '  skills/signaltrim/SKILL.md',
    '',
    'Default intensity: `full`. Switch with `/signaltrim lite|full|ultra|wenyan`.',
    'Stop with: "stop signaltrim" / "normal mode" / "deactivate signaltrim".',
    '',
    'Auto-Clarity: drop signaltrim for security warnings, irreversible action',
...
function loadSkillBody(repoRoot) {
  ...
  return readIfExists(path.join(repoRoot, 'skills', 'signaltrim', 'SKILL.md'));
}
...
function installOpenclaw({ workspace, repoRoot, dryRun = false, force = false, log = noopLog() } = {}) {
  ...
  if (!skillBody) {
    log.warn('  openclaw install requires the signaltrim repo on disk (skills/signaltrim/SKILL.md missing).');
    log.note('  Re-run from a clone or via `npx -y github:karurikwao/signaltrim -- --only openclaw`.');
  ...
  if (dryRun) {
    ...
    log.note(`  would ${fs.existsSync(soulFile) ? 'append to' : 'create'} ${soulFile} (signaltrim bootstrap block)`);
  ...
  else log.note(`  ${soulFile} already contains signaltrim bootstrap`);
...
function uninstallOpenclaw({ workspace, dryRun = false, log = noopLog() } = {}) {
  ...
  if (fs.existsSync(soulFile)) {
    if (dryRun) {
      log.note(`  would strip signaltrim block from ${soulFile}`);
      touched = true;
    } else {
      const r = stripBootstrapFromSoul(soulFile);
      if (r.changed) {
        log.note(r.removed ? `  removed ${soulFile}` : `  stripped signaltrim block from ${soulFile}`);
        touched = true;
      }
    }
  }
