// signaltrim — JSONC-tolerant settings.json read/write + defensive hook validation.
//
// Lifted in spirit from gsd-build/get-shit-done's stripJsonComments + readSettings.
// Reused by bin/install.js and (optionally) by hooks/signaltrim-activate.js so a
...
//   hasSignalTrimHook(settings, ev)   → idempotency probe
//   addCommandHook(settings, ev, opts) → no-op if substring marker already present
//   removeSignalTrimHooks(settings)   → uninstall helper
//
...
function readSettings(p) {
  ...
  try { raw = fs.readFileSync(p, 'utf8'); }
    ...
    process.stderr.write(`signaltrim: cannot read ${p}: ${e.message}\n`);
    return null;
  }
  if (!raw.trim()) return {};
  ...
  try { return JSON.parse(stripJsonComments(raw)); }
    ...
    process.stderr.write(`signaltrim: warning — ${p} is not valid JSON or JSONC: ${e.message}\n`);
    return null;
  }
}
...
function validateHookFields(settings) {
  ...
  for (const ev of Object.keys(settings.hooks)) {
    ...
    settings.hooks[ev] = arr.filter(entry => {
      ...
      if ('matcher' in entry && typeof entry.matcher !== 'string') delete entry.matcher;
      if (!Array.isArray(entry.hooks)) return false;
      entry.hooks = entry.hooks.filter(h => {
        if (!h || typeof h !== 'object') return false;
        if ('timeout' in h && typeof h.timeout !== 'number') delete h.timeout;
        if ('statusMessage' in h && typeof h.statusMessage !== 'string') delete h.statusMessage;
...
// ── Idempotency probe ──────────────────────────────────────────────────────
function hasSignalTrimHook(settings, event, marker = 'signaltrim') {
...
function addCommandHook(settings, event, opts) {
  ...
  if (hasSignalTrimHook(settings, event, marker)) return false;
...
// against a bare "signaltrim" substring, which also matches user-authored hooks
...
// match — not substring — so `mysignaltrim-activate.js` or a user hook living
// under a `signaltrim-notes/` directory is never treated as ours. win32.basename
...
function removeSignalTrimHooks(settings) {
...
// on user-authored hooks that just happen to mention "signaltrim".
...
// signaltrim-*.js) to the Claude Code plugin disables/renames those local
...
// …signaltrim-activate.js` (issue #471). rewriteLegacyManagedHookCommands can't
// help — it only matches the bare-node shape and these orphans are usually
// absolute-node — and removeSignalTrimHooks runs only on uninstall.
//
...
function pruneOrphanedManagedHooks(settings, configDir) {
  ...
  // `mysignaltrim-activate.js` is never touched) and that resolved path is absent.
