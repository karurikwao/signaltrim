...
- The installer shells out to per-agent CLIs which fetch from their own registries/sources: `claude plugin marketplace add` / `claude plugin install` (Anthropic/GitHub), `gemini extensions install`, `npx -y --package github:karurikwao/signaltrim signaltrim-shrink ...` (GitHub/npm), `npx -y skills add` (npm).
- **Rare fallback:** if the installer runs detached from a repo checkout/package, it downloads hook files from raw.githubusercontent.com at `SIGNALTRIM_REF` or `main`. It verifies downloaded hooks against `src/hooks/checksums.sha256` from the same ref when that manifest is available; a mismatch aborts. If no manifest is available, it warns and installs downloaded hooks unverified. From a normal clone or npx run, files are copied locally.
...
- **Windows Defender / SmartScreen on `install.ps1` (#383):** piping a script from the internet into `iex` and writing into agent config directories matches generic dropper heuristics, so AV tools may warn. The script is short and readable in this repo; normal clone/npx installs copy bundled hook files locally, while detached fallback downloads use `SIGNALTRIM_REF` or `main` and verify against `src/hooks/checksums.sha256` from that same ref when available. A checksum mismatch aborts; a missing manifest triggers an unverified-download warning. If you'd rather not pipe-to-shell, clone the repo and run `node bin/install.js` — same result, fully inspectable first.
