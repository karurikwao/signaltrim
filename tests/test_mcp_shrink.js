#!/usr/bin/env node
// Tests for src/mcp-servers/signaltrim-shrink/compress.js — pure-Node prose compressor.
...
const { compress, compressDescriptionsInPlace } = require(
  path.join(ROOT, 'src', 'mcp-servers', 'signaltrim-shrink', 'compress.js')
);
const { getSpawnOptions } = require(
  path.join(ROOT, 'src', 'mcp-servers', 'signaltrim-shrink', 'spawn-options.js')
);
const { assertSafeSpawnInput } = require(
  path.join(ROOT, 'src', 'mcp-servers', 'signaltrim-shrink', 'spawn-options.js')
);
...
test('win32 rejects shell metacharacters before upstream spawn', () => {
  assert.throws(
    () => assertSafeSpawnInput('npx', ['@modelcontextprotocol/server-filesystem', 'docs&calc'], 'win32'),
    /unsafe Windows shell argument/
  );
  assert.doesNotThrow(() => {
    assertSafeSpawnInput('npx', ['@modelcontextprotocol/server-filesystem', 'C:\\Users\\me\\docs'], 'win32');
  });
});

test('posix does not apply cmd.exe metacharacter policy', () => {
  assert.doesNotThrow(() => {
    assertSafeSpawnInput('sh', ['-c', 'printf ok && true'], 'linux');
  });
});
...
test('package.json "files" ships every module the entry points require (#597)', () => {
  const pkgDir = path.join(ROOT, 'src', 'mcp-servers', 'signaltrim-shrink');
