...
// signaltrim skill dirs into the `productivity/` category. `--only hermes` makes
...
const SKILLS = ['signaltrim', 'signaltrim-commit', 'signaltrim-review', 'signaltrim-help', 'signaltrim-stats', 'signaltrim-compress', 'signalteam'];

function freshHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'signaltrim-hermes-'));
}
...
test('hermes fresh install lands 7 skill dirs with SKILL.md under skills/productivity/', () => {
  ...
  try {
    ...
    // signaltrim-compress ships executable scripts — ensure the recursive copy kept them.
    assert.ok(fs.existsSync(path.join(prod, 'signaltrim-compress', 'scripts')), 'signaltrim-compress/scripts/ not copied');
  } finally {
...
test('hermes uninstall removes all installed signaltrim skills (no orphans)', () => {
  const home = freshHome();
  try {
