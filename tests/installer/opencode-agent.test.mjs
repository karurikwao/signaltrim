...
// Claude Code accepts. Copying agents/signalteam-*.md verbatim into
// ~/.config/opencode/agents/ broke opencode startup with:
//   Configuration is invalid at .../signalteam-reviewer.md
...
const SHIPPED_AGENT_FILES = ['signalteam-investigator.md', 'signalteam-builder.md', 'signalteam-reviewer.md'];
...
// This is the RED-state proof: each `agents/signalteam-*.md` in the repo today
...
test('all shipped signalteam agent files contain offending tools array (RED proof)', () => {
...
test('all shipped signalteam agent files become opencode-safe after transform (GREEN proof)', () => {
  for (const f of SHIPPED_AGENT_FILES) {
    const src = fs.readFileSync(path.join(REPO_ROOT, 'agents', f), 'utf8');
    const out = stripOpencodeAgentTools(src);
    const fm = frontmatter(out);
    ...
    assert.match(fm, /^name: signalteam-/m, `${f}: name field preserved`);
