...
function freshTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'signaltrim-freshinstall-'));
}
...
function signaltrimHookCommands(settings, event, marker) {
...
test('fresh install populates hooks dir and settings.json (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; the claude provider is the only path that wires hooks' }, () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(hooks, 'signaltrim-activate.js')),     'signaltrim-activate.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'signaltrim-mode-tracker.js')), 'signaltrim-mode-tracker.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'signaltrim-config.js')),       'signaltrim-config.js missing');
    ...
    assert.ok(SETTINGS.hasSignalTrimHook(settings, 'SessionStart', 'signaltrim-activate'),
      'SessionStart hook missing or wrong marker');
    assert.ok(SETTINGS.hasSignalTrimHook(settings, 'UserPromptSubmit', 'signaltrim-mode-tracker'),
      'UserPromptSubmit hook missing or wrong marker');
    assert.ok(settings.statusLine, 'statusLine not set');
    assert.match(getStatuslineCommand(settings), /signaltrim-statusline/,
      'statusLine command does not reference signaltrim');
  } finally {
...
test('idempotent install does not duplicate hook entries (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH' }, () => {
  ...
  try {
    ...
    const sessStart = signaltrimHookCommands(settings, 'SessionStart', 'signaltrim-activate');
    assert.equal(sessStart.length, 1, `expected 1 SessionStart signaltrim hook, got ${sessStart.length}`);

    const ups = signaltrimHookCommands(settings, 'UserPromptSubmit', 'signaltrim-mode-tracker');
    assert.equal(ups.length, 1, `expected 1 UserPromptSubmit signaltrim hook, got ${ups.length}`);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('uninstall strips signaltrim hooks but preserves user-authored ones (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; uninstall test depends on a prior real install' }, () => {
  const dir = freshTmpDir();
  try {
    ...
    if (fs.existsSync(hooks)) {
      for (const f of ['signaltrim-activate.js', 'signaltrim-mode-tracker.js', 'signaltrim-config.js', STATUSLINE_FILE]) {
    ...
    // Settings cleaned up.
    const settings = JSON.parse(fs.readFileSync(path.join(dir, 'settings.json'), 'utf8'));
    // No remaining signaltrim-marked hooks anywhere.
    for (const ev of Object.keys(settings.hooks || {})) {
      const arr = settings.hooks[ev] || [];
      for (const e of arr) {
        for (const h of (e.hooks || [])) {
          assert.doesNotMatch(h.command || '', /signaltrim/, `${ev} still has signaltrim hook: ${h.command}`);
        }
      }
    }
    // User's pre-existing hook preserved.
    const preservedUser = signaltrimHookCommands(settings, 'SessionStart', 'user-owned-hook').length > 0;
    assert.ok(preservedUser, 'user-authored SessionStart hook was wiped during uninstall');

    // Statusline pointing at signaltrim should be removed.
    assert.doesNotMatch(getStatuslineCommand(settings), /signaltrim-statusline/,
      'signaltrim statusline survived uninstall');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
`);
    ...
    // SignalTrim hooks must be wired.
    assert.ok(SETTINGS.hasSignalTrimHook(parsed, 'SessionStart', 'signaltrim-activate'));
    assert.ok(SETTINGS.hasSignalTrimHook(parsed, 'UserPromptSubmit', 'signaltrim-mode-tracker'));
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
const SKILL_BODY_SRC = path.join(REPO_ROOT, 'skills', 'signaltrim', 'SKILL.md');
...
test('openclaw install writes skill folder + SOUL.md bootstrap', () => {
  ...
  try {
    ...
    const skillFile = path.join(ws, 'skills', 'signaltrim', 'SKILL.md');
    ...
    assert.match(soulRaw, /<!-- signaltrim-begin -->/, 'SOUL.md missing begin marker');
    assert.match(soulRaw, /<!-- signaltrim-end -->/, 'SOUL.md missing end marker');
    assert.match(soulRaw, /SignalTrim compact mode active/, 'SOUL.md missing sentinel');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw install is idempotent: skill frontmatter not double-prepended, SOUL.md has one marker block', () => {
  ...
  try {
    ...
    const skillRaw = fs.readFileSync(path.join(ws, 'skills', 'signaltrim', 'SKILL.md'), 'utf8');
    ...
    const beginMatches = soulRaw.match(/<!-- signaltrim-begin -->/g) || [];
...
test('openclaw install preserves user content in SOUL.md (append, not overwrite)', () => {
  ...
  try {
    ...
    assert.match(soulRaw, /<!-- signaltrim-begin -->/, 'signaltrim block not appended');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw uninstall removes skill folder + strips SOUL.md block, preserving user content', () => {
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(ws, 'skills', 'signaltrim')), false, 'skill folder should be removed');
    const soulAfter = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.doesNotMatch(soulAfter, /<!-- signaltrim-begin -->/, 'signaltrim block survived uninstall');
    assert.doesNotMatch(soulAfter, /<!-- signaltrim-end -->/, 'signaltrim end marker survived uninstall');
...
test('signaltrim-init.js --only openclaw routes through the same helper', () => {
  const dir = freshTmpDir();
  const ws = path.join(dir, 'ws');
  fs.mkdirSync(ws);
  try {
    const initScript = path.join(REPO_ROOT, 'src', 'tools', 'signaltrim-init.js');
    const r = spawnSync('node', [initScript, dir, '--only', 'openclaw'], {
      env: { ...process.env, OPENCLAW_WORKSPACE: ws, NO_COLOR: '1' },
      encoding: 'utf8',
    });
    assert.equal(r.status, 0, `signaltrim-init failed: ${r.stderr || r.stdout}`);
    assert.ok(fs.existsSync(path.join(ws, 'skills', 'signaltrim', 'SKILL.md')), 'skill missing via init route');
    assert.ok(fs.existsSync(path.join(ws, 'SOUL.md')), 'SOUL.md missing via init route');
    const soulRaw = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.match(soulRaw, /SignalTrim compact mode active/, 'sentinel missing via init route');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('lib settings.addCommandHook is idempotent across two synthetic install passes', () => {
  ...
  try {
    ...
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/abs/hooks/signaltrim-activate.js"',
      marker: 'signaltrim-activate',
    });
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/different/hooks/signaltrim-activate.js"',
      marker: 'signaltrim-activate',
    });
...
test('opencode: --force on legacy AGENTS.md preserves user content and takes a backup', () => {
  ...
  const legacyBody = fs.readFileSync(
    path.join(REPO_ROOT, 'src', 'rules', 'signaltrim-activate.md'), 'utf8').trimEnd() + '\n';
  ...
  try {
    ...
    assert.match(after, /<!-- signaltrim-begin -->/, 'fenced block missing after migration');
    assert.match(after, /<!-- signaltrim-end -->/, 'fence end missing after migration');
    // Legacy un-fenced copy must be gone: sentinel appears only inside the fence.
    const beforeFence = after.slice(0, after.indexOf('<!-- signaltrim-begin -->'));
    assert.doesNotMatch(beforeFence, /SignalTrim compact mode active/,
...
test('openclaw: truncated begin marker does not eat user content (issue #596 chain)', () => {
  ...
  try {
    ...
    assert.doesNotMatch(afterStrip, /signaltrim-begin/, 'marker survived strip');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw: strip removes multiple blocks pairwise, keeping user content between them', () => {
  ...
  try {
    ...
    assert.doesNotMatch(after, /signaltrim-(begin|end)/, 'markers survived');
...
test('openclaw: orphan end marker stripped without touching content', () => {
  ...
  try {
    ...
    assert.doesNotMatch(after, /signaltrim-end/);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('missing claude CLI: reports failure and falls back to standalone hook wiring', () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(configDir, 'hooks', 'signaltrim-activate.js')),
