...
test('--list prints provider matrix', () => {
  const r = run('--list');
  assert.equal(r.status, 0);
  assert.match(r.stdout, /SignalTrim provider matrix/);
...
test('--only with unknown agent id exits 2', () => {
  ...
  assert.match(r.stderr, /signaltrim --list/);
});
...
test('--config-dir expands ~ to home directory', async () => {
  ...
  // match even when the signaltrim plugin is already installed.
  ...
  if (/Claude Code detected/.test(r.stdout)) {
    ...
    const expected = path.join(homedir(), suffix);
    assert.match(r.stdout, new RegExp(expected.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
  }
});
...
test('bare --with-mcp-shrink (no upstream) exits 2 with hint', () => {
  ...
  // that crashed on every Claude Code startup. signaltrim-shrink is a proxy and
...
test('--all does NOT auto-enable mcp-shrink (no sensible default upstream)', () => {
  ...
  assert.doesNotMatch(r.stdout, /wiring signaltrim-shrink MCP proxy/);
});
