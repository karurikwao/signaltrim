...
test('validateHookFields strips malformed optional hook fields', () => {
  const s = {
    hooks: {
      SessionStart: [{
        matcher: [],
  ...
  assert.equal('timeout' in hook, false);
  assert.equal('statusMessage' in hook, false);
  assert.equal(hook.command, 'good');
});
...
test('addCommandHook is idempotent on substring marker', () => {
  const s = {};
  const a = SETTINGS.addCommandHook(s, 'SessionStart', { command: '/abs/path/signaltrim-activate.js', marker: 'signaltrim-activate' });
  const b = SETTINGS.addCommandHook(s, 'SessionStart', { command: '/different/abs/path/signaltrim-activate.js', marker: 'signaltrim-activate' });
  assert.equal(a, true);
  assert.equal(b, false);
  assert.equal(s.hooks.SessionStart.length, 1);
});

test('hasSignalTrimHook detects via substring', () => {
  const s = { hooks: { SessionStart: [{ hooks: [{ type: 'command', command: 'node /x/signaltrim-activate.js' }] }] } };
  assert.equal(SETTINGS.hasSignalTrimHook(s, 'SessionStart', 'signaltrim-activate'), true);
  assert.equal(SETTINGS.hasSignalTrimHook(s, 'SessionStart', 'gsd'), false);
  assert.equal(SETTINGS.hasSignalTrimHook(s, 'UserPromptSubmit'), false);
});

test('removeSignalTrimHooks tolerates malformed hook event values without throwing', () => {
  ...
  let removed;
  assert.doesNotThrow(() => { removed = SETTINGS.removeSignalTrimHooks(s); });
  assert.equal(removed, 0);
  assert.equal(s.hooks, undefined);
});

test('removeSignalTrimHooks strips managed scripts and cleans empties', () => {
  const s = {
    hooks: {
      SessionStart: [
        { hooks: [{ type: 'command', command: 'node /x/hooks/signaltrim-activate.js' }] },
        { hooks: [{ type: 'command', command: 'other' }] },
      ],
      UserPromptSubmit: [{ hooks: [{ type: 'command', command: '"/usr/bin/node" "/x/hooks/signaltrim-mode-tracker.js"' }] }],
    },
  };
...
test('removeSignalTrimHooks leaves user hooks that merely mention signaltrim (issue #593)', () => {
  const s = {
    hooks: {
      PreToolUse: [
        // Path contains the word "signaltrim" but targets a user-authored script.
        { hooks: [{ type: 'command', command: 'node /Users/me/Projects/signaltrim-notes/my-hook.js' }] },
        // Basename is a superstring of a managed name — still not ours.
        { hooks: [{ type: 'command', command: 'node /x/mysignaltrim-activate.js' }] },
      ],
      SessionStart: [
        { hooks: [{ type: 'command', command: '"/usr/bin/node" "/x/hooks/signaltrim-activate.js"' }] },
      ],
    },
  };
  const removed = SETTINGS.removeSignalTrimHooks(s);
  assert.equal(removed, 1, 'only the managed SessionStart hook should be removed');
  assert.equal(s.hooks.PreToolUse.length, 2, 'user hooks mentioning signaltrim must survive uninstall');
...
test('removeSignalTrimHooks removes the Windows statusline-stats wiring (signaltrim-stats.js / .ps1)', () => {
  const s = {
    hooks: {
      Stop: [{ hooks: [{ type: 'command', command: '"C:\\Program Files\\nodejs\\node.exe" "C:\\Users\\me\\.claude\\hooks\\signaltrim-stats.js"' }] }],
    },
  };
  const removed = SETTINGS.removeSignalTrimHooks(s);
  assert.equal(removed, 1);
...
test('rewriteLegacyManagedHookCommands rewrites bare-node managed scripts', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: 'node /abs/hooks/signaltrim-activate.js' },
  ...
  assert.equal(n, 1);
  assert.match(s.hooks.SessionStart[0].hooks[0].command, /"\/usr\/local\/bin\/node" "\/abs\/hooks\/signaltrim-activate\.js"/);
...
test('rewriteLegacyManagedHookCommands ignores already-absolute node commands', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: '"/usr/local/bin/node" "/abs/hooks/signaltrim-activate.js"' },
      ] }],
    },
  };
...
test('pruneOrphanedManagedHooks removes managed hook whose target is missing (absolute-node)', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: '"/opt/node/bin/node" "/no/such/dir/signaltrim-activate.js"' },
      ] }],
    },
  };
  ...
  const s = {
    hooks: {
      UserPromptSubmit: [{ hooks: [
        { type: 'command', command: 'node /no/such/dir/signaltrim-mode-tracker.js' },
      ] }],
    },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, '/tmp/__cm_cfg_missing');
...
test('pruneOrphanedManagedHooks removes Windows-style managed hook path', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: 'node C:\\Users\\me\\.claude\\hooks\\signaltrim-activate.js' },
      ] }],
    },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, '/tmp/__cm_cfg_missing');
...
test('pruneOrphanedManagedHooks keeps managed hook whose target exists', () => {
  ...
  const script = path.join(dir, 'signaltrim-activate.js');
...
test('pruneOrphanedManagedHooks resolves relative target against configDir', () => {
  ...
  // hooks/signaltrim-activate.js intentionally NOT created → missing
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        { type: 'command', command: 'node hooks/signaltrim-activate.js' },
      ] }],
    },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, dir);
...
test('pruneOrphanedManagedHooks does NOT match a user script whose name merely contains a managed basename', () => {
  const s = {
    hooks: {
      SessionStart: [{ hooks: [
        // basename is "mysignaltrim-activate.js" — not an exact managed basename
        { type: 'command', command: 'node /no/such/dir/mysignaltrim-activate.js' },
      ] }],
    },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, '/tmp/__cm_cfg_missing');
...
test('pruneOrphanedManagedHooks drops orphaned managed statusLine', () => {
  const s = {
    statusLine: { type: 'command', command: 'bash /no/such/dir/signaltrim-statusline.sh' },
  };
  const removed = SETTINGS.pruneOrphanedManagedHooks(s, '/tmp/__cm_cfg_missing');
  assert.equal(removed, 1);
