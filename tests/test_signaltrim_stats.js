#!/usr/bin/env node
// Tests for /signaltrim-stats — direct script invocation and via mode tracker.
// Run: node tests/test_signaltrim_stats.js
...
const STATS = path.join(ROOT, 'src', 'hooks', 'signaltrim-stats.js');
const TRACKER = path.join(ROOT, 'src', 'hooks', 'signaltrim-mode-tracker.js');

let passed = 0;
let failed = 0;

function test(name, fn) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'signaltrim-stats-test-'));
  try {
    fn(tmp);
    passed++;
    console.log(`  ✓ ${name}`);
...
console.log('signaltrim-stats tests\n');
...
test('shows full-mode savings estimate when flag is full', (tmp) => {
  ...
  fs.writeFileSync(path.join(claudeDir, '.signaltrim-active'), 'full');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
  // 350 / 0.35 = 1000, saved = 650, ~65%
  assert.match(out, /Est\. without signaltrim:\s+1,000/);
...
test('skips estimate for non-full modes', (tmp) => {
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.signaltrim-active'), 'ultra');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
...
test('mode tracker handles /signaltrim-stats with decision block', (tmp) => {
  const sess = makeSession(tmp, [
    { type: 'assistant', message: { usage: { output_tokens: 100 } } },
  ]);
  const claudeDir = path.join(tmp, '.claude');
  ...
  const out = execFileSync(process.execPath, [TRACKER], {
    ...
    input: JSON.stringify({ prompt: '/signaltrim-stats', transcript_path: sess }),
  });
  const parsed = JSON.parse(out);
  assert.strictEqual(parsed.decision, 'block');
  assert.match(parsed.reason, /SignalTrim Stats/);
  assert.match(parsed.reason, /Output tokens:\s+100/);
});

test('mode tracker preserves signaltrim flag when /signaltrim-stats fires', (tmp) => {
  const sess = makeSession(tmp, [
  ...
  assert.strictEqual(fs.readFileSync(path.join(claudeDir, '.signaltrim-active'), 'utf8'), 'full');
});
...
test('priceForModel matches by prefix across point releases', () => {
  const { priceForModel } = require(path.join(ROOT, 'src', 'hooks', 'signaltrim-stats.js'));
...
test('formatStats handles empty session gracefully', () => {
  const { formatStats } = require(path.join(ROOT, 'src', 'hooks', 'signaltrim-stats.js'));
...
test('--share prints single-line tweetable summary', (tmp) => {
  ...
  assert.match(out, /^SignalTrim saved 650 output tokens \(~\$0\.009[78]\) across 1 turns this session - signaltrim\.sh$/m);
});
...
test('--share works with no benchmark ratio (lite mode)', (tmp) => {
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.signaltrim-active'), 'lite');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess, '--share'], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
  assert.match(out, /^SignalTrim 1 turns, 200 output tokens this session - signaltrim\.sh$/m);
});
...
test('appends to lifetime history on each run', (tmp) => {
  ...
  const histPath = path.join(claudeDir, '.signaltrim-history.jsonl');
...
test('skips pairs where compressed is not actually smaller', (tmp) => {
  const { findCompressedPairs } = require(path.join(ROOT, 'src', 'hooks', 'signaltrim-stats.js'));
...
test('writes statusline suffix file after a stats run', (tmp) => {
  ...
  const suffixPath = path.join(claudeDir, '.signaltrim-statusline-suffix');
  assert.ok(fs.existsSync(suffixPath));
  // 1500 / 0.35 = 4286, saved = 2786 -> "saved 2.8k"
  const suffix = fs.readFileSync(suffixPath, 'utf8');
  assert.match(suffix, /^saved 2\.8k$/);
});

test('humanizeTokens formats small/medium/large correctly', () => {
  const { humanizeTokens } = require(path.join(ROOT, 'src', 'hooks', 'signaltrim-stats.js'));
...
test('statusline.sh appends savings when SIGNALTRIM_STATUSLINE_SAVINGS=1', (tmp) => {
  if (process.platform === 'win32') return; // bash test
  const claudeDir = path.join(tmp, '.claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.writeFileSync(path.join(claudeDir, '.signaltrim-active'), 'full');
  fs.writeFileSync(path.join(claudeDir, '.signaltrim-statusline-suffix'), 'saved 2.8k');
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'signaltrim-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, SIGNALTRIM_STATUSLINE_SAVINGS: '1' },
  });
  assert.match(out, /\[SIGNALTRIM\]/);
  assert.match(out, /saved 2\.8k/);
});
...
test('statusline.sh renders savings by default when env var is unset', (tmp) => {
  ...
  delete env.SIGNALTRIM_STATUSLINE_SAVINGS;
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'signaltrim-statusline.sh')], {
    encoding: 'utf8', env,
  });
  assert.match(out, /\[SIGNALTRIM\]/);
...
test('statusline.sh omits savings when SIGNALTRIM_STATUSLINE_SAVINGS=0', (tmp) => {
  if (process.platform === 'win32') return;
  const claudeDir = path.join(tmp, '.claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.writeFileSync(path.join(claudeDir, '.signaltrim-active'), 'full');
  ...
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'signaltrim-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, SIGNALTRIM_STATUSLINE_SAVINGS: '0' },
  });
  assert.match(out, /\[SIGNALTRIM\]/);
  assert.doesNotMatch(out, /saved 2\.8k/);
});
...
test('statusline.sh omits savings when suffix file is missing (fresh install)', (tmp) => {
  ...
  // /signaltrim-stats has run. Default-on must NOT fabricate a number.
  const env = { ...process.env, CLAUDE_CONFIG_DIR: claudeDir };
  delete env.SIGNALTRIM_STATUSLINE_SAVINGS;
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'signaltrim-statusline.sh')], {
    encoding: 'utf8', env,
...
test('statusline.sh strips control bytes from suffix', (tmp) => {
  ...
  fs.writeFileSync(path.join(claudeDir, '.signaltrim-statusline-suffix'), '\x1b[31mEVIL');
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'signaltrim-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, SIGNALTRIM_STATUSLINE_SAVINGS: '1' },
  });
...
test('appendFlag is symlink-safe (refuses symlinked target)', (tmp) => {
  ...
  const { appendFlag } = require(path.join(ROOT, 'src', 'hooks', 'signaltrim-config.js'));
...
test('mode tracker forwards --share to stats script', (tmp) => {
  ...
  const out = execFileSync(process.execPath, [TRACKER], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, HOME: tmp },
    input: JSON.stringify({ prompt: '/signaltrim-stats --share', transcript_path: sess }),
  });
  const parsed = JSON.parse(out);
  assert.strictEqual(parsed.decision, 'block');
  assert.match(parsed.reason, /^SignalTrim saved 650 output tokens/);
});
...
test('--all lifetime output labels the % as output reduction, not usage', (tmp) => {
  ...
  fs.writeFileSync(
    path.join(claudeDir, '.signaltrim-history.jsonl'),
...
// via the .signaltrim-mode-log.jsonl transition log — never to whatever mode the
...
test('attributes tokens to the mode active when each message happened (#601)', (tmp) => {
  ...
  // 300 verbose tokens BEFORE signaltrim was activated, 350 after.
  const sess = makeSession(tmp, [
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.signaltrim-mode-log.jsonl'),
  ...
  assert.match(out, /signaltrim off:\s+300 tokens \(no benchmark estimate\)/);
  ...
  const hist = fs.readFileSync(path.join(claudeDir, '.signaltrim-history.jsonl'), 'utf8')
    .split('\n').filter(Boolean).map(l => JSON.parse(l));
  assert.strictEqual(hist[hist.length - 1].est_saved_tokens, 650);
});

test('credits signaltrim spans even after mode is turned off mid-session (#601)', (tmp) => {
  const now = Date.now();
  const iso = (minAgo) => new Date(now - minAgo * 60_000).toISOString();
  const sess = makeSession(tmp, [
  ...
  // No .signaltrim-active flag — signaltrim is off at stats time. The old behavior
  // printed "SignalTrim not active this session." and logged zero savings.
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
  assert.doesNotMatch(out, /SignalTrim not active this session/);
  assert.match(out, /full:\s+350 tokens \(est\. 650 saved\)/);
  assert.match(out, /Est\. tokens saved:\s+650\b/);
});
...
test('mode tracker logs timestamped transitions, deduping unchanged modes (#601)', (tmp) => {
  ...
  const logPath = path.join(claudeDir, '.signaltrim-mode-log.jsonl');
  const rows = () => fs.readFileSync(logPath, 'utf8').split('\n').filter(Boolean).map(l => JSON.parse(l));

  run('/signaltrim ultra');
  run('/signaltrim ultra'); // unchanged — must not append a duplicate row
  assert.strictEqual(rows().length, 1);
  assert.strictEqual(rows()[0].mode, 'ultra');
  assert.strictEqual(rows()[0].prev, 'full');
  assert.ok(Number.isFinite(rows()[0].ts));

  run('/signaltrim off'); // deactivation is a transition too
...
test('excludes tokens that predate a mid-session flag write with no log (#601)', (tmp) => {
  ...
  assert.doesNotMatch(out, /Est\. without signaltrim/);
});
