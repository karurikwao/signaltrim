#!/usr/bin/env node
// Tests for the stdin 'error' handler in signaltrim-mode-tracker.js.
...
const HOOK_PATH = path.resolve(__dirname, '..', 'src', 'hooks', 'signaltrim-mode-tracker.js');
const CLEAN_EXIT = 0;
...
console.log('signaltrim-mode-tracker stdin error handling\n');
...
test('normal stdin (valid JSON + clean EOF) still exits 0', () => {
  const tmpConfig = fs.mkdtempSync(path.join(os.tmpdir(), 'signaltrim-tracker-stdin-'));
  try {
