...
Imports: organized with external, internal, and relative groups separated. Use path aliases (`@/components/...`) not deep relative paths.
...
React: Testing Library, no internal state or lifecycle testing. API endpoints: integration tests hit real DB — mocked tests passed but prod broke too many times.

No 100% coverage needed. Critical paths matter. Services: thorough unit tests. UI: happy path + key error states.

## Communication Style

Senior engineer, 2 years on project. Skip basic concepts. Concise, direct. Tradeoffs: options plus pros and cons, no paragraphs.
...
No new deps without discussion. Keep bundle small, avoid duplicate libraries. Prefer built-in Node or browser APIs over packages.
...
Never commit `.env` or secrets, keys, or credentials. `.env.example` gets placeholder values only.
