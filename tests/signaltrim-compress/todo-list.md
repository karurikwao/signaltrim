...
- [ ] **TF-478: Add Slack notification integration** — Notify Slack channel on task assignment or status change. Webhook infra ready. Wire event handlers in task service + Slack msg formatting. Jordan Kim. Due Apr 18.
...
- [ ] **TF-510: Investigate Playwright test flakiness** — E2E drag-and-drop reorder fails about once every five CI runs. Timing issue with animation completion detection. Not blocking, but hurts test confidence. Unassigned.
...
- [x] **TF-451: Add rate limiting to public API endpoints** — Alex, Apr 3. `express-rate-limit` with Redis. 100 requests per minute authed, 20 unauthed.
