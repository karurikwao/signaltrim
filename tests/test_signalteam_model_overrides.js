#!/usr/bin/env node
// Tests for src/hooks/signalteam-model-overrides.js
// Run: node tests/test_signalteam_model_overrides.js
...
const { patchFrontmatterModel, resolvePluginRoot, applyOverrides, AGENT_ENV_MAP } =
  require('../src/hooks/signalteam-model-overrides');
...
test('preserves all other frontmatter lines', () => {
  ...
  assert.ok(out.includes('name: signalteam-reviewer'), 'name line lost');
...
test('prefers CLAUDE_PLUGIN_ROOT when it contains agents dir', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'signaltrim-root-env-'));
  try {
    fs.mkdirSync(path.join(tmp, 'agents'));
    const nestedHooks = path.join(os.tmpdir(), 'wrong-plugin', 'src', 'hooks');
    const root = resolvePluginRoot(nestedHooks, { CLAUDE_PLUGIN_ROOT: tmp });
...
test('resolves shipped src/hooks layout to plugin root', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'signaltrim-root-src-'));
  try {
...
function withTmpPlugin(fn) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'signaltrim-override-test-'));
...
test('replaces reviewer model when SIGNALTEAM_REVIEWER_MODEL set', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { SIGNALTEAM_REVIEWER_MODEL: 'sonnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), 'utf8');
    assert.ok(out.includes('model: sonnet'), 'reviewer model not patched');
  });
});

test('applies overrides after resolving shipped src/hooks layout', () => {
  withTmpPlugin((root, agentsDir) => {
    const hooksDir = path.join(root, 'src', 'hooks');
    fs.mkdirSync(hooksDir, { recursive: true });
    fs.writeFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), REVIEWER_FM, 'utf8');

    const resolved = resolvePluginRoot(hooksDir, {});
    applyOverrides(resolved, { SIGNALTEAM_REVIEWER_MODEL: 'sonnet' });

    const out = fs.readFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), 'utf8');
    assert.ok(out.includes('model: sonnet'), 'reviewer model not patched from src/hooks layout');
  });
});

test('replaces investigator model when SIGNALTEAM_INVESTIGATOR_MODEL set', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'signalteam-investigator.md'), INVESTIGATOR_FM, 'utf8');
    applyOverrides(root, { SIGNALTEAM_INVESTIGATOR_MODEL: 'opus' });
    const out = fs.readFileSync(path.join(agentsDir, 'signalteam-investigator.md'), 'utf8');
    assert.ok(out.includes('model: opus'), 'investigator model not patched');
  });
});

test('inserts builder model when SIGNALTEAM_BUILDER_MODEL set and no model line', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'signalteam-builder.md'), BUILDER_FM, 'utf8');
    applyOverrides(root, { SIGNALTEAM_BUILDER_MODEL: 'sonnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'signalteam-builder.md'), 'utf8');
...
test('blank env var is no-op', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { SIGNALTEAM_REVIEWER_MODEL: '' });
    const out = fs.readFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), 'utf8');
...
test('env var with newline in value is ignored', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { SIGNALTEAM_REVIEWER_MODEL: 'so\nnnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), 'utf8');
...
test('env var with control character in value is ignored', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { SIGNALTEAM_REVIEWER_MODEL: 'son\x00net' });
    const out = fs.readFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), 'utf8');
...
test('missing agents dir (non-plugin layout) is silent no-op', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'signaltrim-nolayout-'));
  try {
    assert.doesNotThrow(() => {
      applyOverrides(tmp, { SIGNALTEAM_REVIEWER_MODEL: 'sonnet' });
    });
  } finally {
    fs.rmSync(tmp, { recursive: true, force: true });
  }
...
test('body content preserved after model patch', () => {
  withTmpPlugin((root, agentsDir) => {
    ...
    fs.writeFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), content, 'utf8');
    applyOverrides(root, { SIGNALTEAM_REVIEWER_MODEL: 'sonnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'signalteam-reviewer.md'), 'utf8');
