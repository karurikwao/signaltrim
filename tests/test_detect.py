...
compressible natural language — so `/signaltrim-compress Dockerfile` would
overwrite a Dockerfile with signaltrim prose. These tests pin the basename
and shebang guards.
"""
...
class DetectFileTypeTests(unittest.TestCase):
    ...
    def test_backup_files_are_skipped_case_insensitive(self):
        with tempfile.TemporaryDirectory() as tmp:
            p = self._write(tmp, "TASK.ORIGINAL.MD", PROSE_BODY)
            self.assertFalse(should_compress(p))
