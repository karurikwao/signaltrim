#!/usr/bin/env python3
"""Local verification runner for signaltrim install surfaces."""
...
def verify_skill_frontmatter_upload_compatibility() -> None:
    ...
    skill_paths = [
        ROOT / "skills/signaltrim/SKILL.md",
        ROOT / "skills/signaltrim-commit/SKILL.md",
        ROOT / "skills/signaltrim-help/SKILL.md",
        ROOT / "skills/signaltrim-review/SKILL.md",
        ROOT / "skills/signaltrim-compress/SKILL.md",
    ]
...
def verify_synced_files() -> None:
    ...
    skill_source = ROOT / "skills/signaltrim/SKILL.md"

    skill_copies = [
        ROOT / "plugins/signaltrim/skills/signaltrim/SKILL.md",
    ]
    ...
    with zipfile.ZipFile(ROOT / "dist" / "signaltrim.skill") as archive:
        ensure("signaltrim/SKILL.md" in archive.namelist(), "signaltrim.skill missing signaltrim/SKILL.md")
        ensure(
            archive.read("signaltrim/SKILL.md").decode("utf-8")
            == skill_source.read_text(encoding="utf-8"),
            "signaltrim.skill payload mismatch",
        )

    ensure(
        (ROOT / "bin" / "install.js").exists(),
        "bin/install.js missing — package.json bin entry would break npx signaltrim",
    )
    ensure(
    ...
    print("Synced copies, signaltrim.skill zip, and installer entrypoints OK")
...
def verify_manifests_and_syntax() -> None:
    ...
    manifest_paths = [
        ...
        ROOT / "plugins/signaltrim/.codex-plugin/plugin.json",
    ]
    for path in manifest_paths:
        read_json(path)

    run(["node", "--check", "src/hooks/signaltrim-config.js"])
    run(["node", "--check", "src/hooks/signaltrim-activate.js"])
    run(["node", "--check", "src/hooks/signaltrim-mode-tracker.js"])
    run(["node", "--check", "src/hooks/signalteam-model-overrides.js"])
    run(["node", "--check", "bin/install.js"])
    run(["node", "--check", "bin/lib/settings.js"])
    bash_cmd = find_usable_bash()
    if bash_cmd:
        run([bash_cmd, "-n", "src/hooks/install.sh"])
        run([bash_cmd, "-n", "src/hooks/uninstall.sh"])
        run([bash_cmd, "-n", "src/hooks/signaltrim-statusline.sh"])
    else:
        print("Skipping bash syntax checks (bash unavailable or unusable)")

    # Ensure install/uninstall scripts include signaltrim-config.js
    ...
    ensure("signaltrim-config.js" in install_sh, "install.sh missing signaltrim-config.js")
    ensure("signaltrim-config.js" in uninstall_sh, "uninstall.sh missing signaltrim-config.js")
...
def verify_powershell_static() -> None:
    ...
    statusline_text = (ROOT / "src/hooks/signaltrim-statusline.ps1").read_text(encoding="utf-8")

    ensure("signaltrim-config.js" in install_text, "install.ps1 missing signaltrim-config.js")
    ensure("signaltrim-config.js" in uninstall_text, "uninstall.ps1 missing signaltrim-config.js")
    ensure("signaltrim-statusline.ps1" in install_text, "install.ps1 missing statusline.ps1")
    ensure("signaltrim-statusline.ps1" in uninstall_text, "uninstall.ps1 missing statusline.ps1")
    ...
    ensure("[SIGNALTRIM" in statusline_text, "signaltrim-statusline.ps1 missing badge output")

    print("Windows install path statically wired")


def load_compress_modules():
    sys.path.insert(0, str(ROOT / "skills/signaltrim-compress"))
...
def verify_compress_fixtures() -> None:
    ...
    fixtures = sorted((ROOT / "tests/signaltrim-compress").glob("*.original.md"))
    ensure(fixtures, "No signaltrim-compress fixtures found")
    ...
    print(f"Validated {len(fixtures)} signaltrim-compress fixture pairs")
...
def verify_compress_cli() -> None:
    ...
    skip_result = run(
        ...
        cwd=ROOT / "skills/signaltrim-compress",
        check=False,
    )
...
def verify_hook_install_flow() -> None:
    ...
    bash_cmd = find_usable_bash()
    if not bash_cmd:
        print("Skipping Claude hook flow (bash unavailable or unusable)")
        return

    with tempfile.TemporaryDirectory(prefix="signaltrim-verify-") as temp_root:
        ...
        run([bash_cmd, "src/hooks/install.sh"], env=hook_env)
        ...
        ensure("SIGNALTRIM MODE ACTIVE" in activate.stdout, "activation output missing signaltrim banner")
        ensure("STATUSLINE SETUP NEEDED" not in activate.stdout, "activation should stay quiet when custom statusline exists")
        ensure((claude_dir / ".signaltrim-active").read_text(encoding="utf-8") == "full", "activation flag should default to full")

        # Test configurable default mode via SIGNALTRIM_DEFAULT_MODE env var
        activate_custom = run(
            ["node", "src/hooks/signaltrim-activate.js"],
            env={**hook_env, "SIGNALTRIM_DEFAULT_MODE": "ultra"},
        )
        ensure("SIGNALTRIM MODE ACTIVE" in activate_custom.stdout, "activation with custom default missing banner")
        ensure(
            (claude_dir / ".signaltrim-active").read_text(encoding="utf-8") == "ultra",
            "SIGNALTRIM_DEFAULT_MODE=ultra should set flag to ultra",
        )
        # Test "off" mode — activation skipped, flag removed
        activate_off = run(
            ["node", "src/hooks/signaltrim-activate.js"],
            env={**hook_env, "SIGNALTRIM_DEFAULT_MODE": "off"},
        )
        ensure("SIGNALTRIM MODE ACTIVE" not in activate_off.stdout, "off mode should not emit signaltrim banner")
        ensure(not (claude_dir / ".signaltrim-active").exists(), "off mode should remove flag file")

        # Test mode tracker with /signaltrim when default is off — should NOT write flag
        subprocess.run(
            ["node", "src/hooks/signaltrim-mode-tracker.js"],
            cwd=ROOT,
            env={**os.environ, **hook_env, "SIGNALTRIM_DEFAULT_MODE": "off"},
            text=True,
            encoding="utf-8",
            input='{"prompt":"/signaltrim"}',
            capture_output=True,
            check=True,
        )
        ensure(not (claude_dir / ".signaltrim-active").exists(), "/signaltrim with off default should not write flag")

        # Reset back to full for subsequent tests
        (claude_dir / ".signaltrim-active").write_text("full")
        ...
        ultra_prompt = subprocess.run(
            ...
            text=True,
            encoding="utf-8",
            input='{"prompt":"/signaltrim ultra"}',
            capture_output=True,
            check=True,
        )
        ensure(
            "SIGNALTRIM MODE ACTIVE (ultra)" in ultra_prompt.stdout,
            "mode tracker should emit active-mode reinforcement",
        )
        ensure((claude_dir / ".signaltrim-active").read_text(encoding="utf-8") == "ultra", "mode tracker did not record ultra")
        ...
        ensure(not (claude_dir / ".signaltrim-active").exists(), "normal mode should remove flag file")

        (claude_dir / ".signaltrim-active").write_text("wenyan-ultra")
        statusline = run(
            [bash_cmd, "src/hooks/signaltrim-statusline.sh"],
            env=hook_env,
        )
        ensure("[SIGNALTRIM:WENYAN-ULTRA]" in statusline.stdout, "statusline badge output mismatch")

        reinstall = run([bash_cmd, "src/hooks/install.sh"], env=hook_env)
        ensure("Nothing to do" in reinstall.stdout, "install.sh should be idempotent")

        run([bash_cmd, "src/hooks/uninstall.sh"], env=hook_env)
        settings_after = read_json(claude_dir / "settings.json")
        ensure(settings_after == existing_settings, "uninstall.sh did not restore non-signaltrim settings")
        ensure(not (claude_dir / ".signaltrim-active").exists(), "uninstall.sh should remove flag file")

    with tempfile.TemporaryDirectory(prefix="signaltrim-verify-fresh-") as temp_root:
        ...
        activate = run(["node", "src/hooks/signaltrim-activate.js"], env=hook_env)
