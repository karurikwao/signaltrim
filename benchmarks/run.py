#!/usr/bin/env python3
"""Benchmark signaltrim vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "signaltrim" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, signaltrim_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("signaltrim", signaltrim_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        signaltrim_medians = statistics.median(
            [t["output_tokens"] for t in entry["signaltrim"]]
        )
        savings = 1 - (signaltrim_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "signaltrim_median": int(signaltrim_medians),
    ...
    avg_signaltrim = round(statistics.mean([r["signaltrim_median"] for r in rows]))
...
def format_table(rows, summary):
    lines = [
        "| Task | Normal (tokens) | SignalTrim (tokens) | Saved |",
        "|------|---------------:|----------------:|------:|",
    ]
    for r in rows:
        label = format_prompt_label(r["id"])
        lines.append(
            f"| {label} | {r['normal_median']} | {r['signaltrim_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_signaltrim']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark signaltrim vs normal Claude")
    ...
    signaltrim_system = load_signaltrim_system()
    ...
    results = run_benchmarks(client, args.model, prompts, signaltrim_system, args.trials)
