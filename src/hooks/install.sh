#!/bin/bash
# signaltrim — one-command hook installer for Claude Code
...
# Local/development helper only. Remote installs must use repo-root install.sh,
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the signaltrim hooks (used to merge"
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if SIGNALTRIM_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.SIGNALTRIM_SETTINGS, 'utf8'));
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "SignalTrim hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/signaltrim-activate.js" ]; then
  echo "Reinstalling signaltrim hooks (--force)..."
else
  echo "Installing signaltrim hooks..."
fi
...
for hook in "${HOOK_FILES[@]}"; do
  if [ -n "$SCRIPT_DIR" ] && [ -f "$SCRIPT_DIR/$hook" ]; then
    ...
    echo "ERROR: hook source not found: $hook"
    echo "       Run from a local clone, or use the repo-root installer:"
    echo "       npx -y github:karurikwao/signaltrim -- --only claude --with-hooks"
    exit 1
  fi
...
chmod +x "$HOOKS_DIR/signaltrim-statusline.sh"
...
SIGNALTRIM_SETTINGS="$SETTINGS" SIGNALTRIM_HOOKS_DIR="$HOOKS_DIR" node -e "
  ...
  // SessionStart — auto-load signaltrim rules
  ...
  // UserPromptSubmit — track mode changes when user types /signaltrim commands
  ...
  // Statusline — wire signaltrim badge (report if skipped)
...
echo "  - SessionStart hook: auto-loads signaltrim rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/signaltrim lite, /signaltrim ultra, /signaltrim-commit, etc.)"
echo "  - Statusline badge: shows [SIGNALTRIM] or [SIGNALTRIM:ULTRA] etc."
