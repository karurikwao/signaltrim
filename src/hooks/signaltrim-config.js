#!/usr/bin/env node
// signaltrim — shared configuration resolver
//
// Resolution order for default mode:
//   1. SIGNALTRIM_DEFAULT_MODE environment variable
//   2. Repo-local config (checked-in, per-project default):
//      - <cwd>/.signaltrim/config.json
//      - <cwd>/.signaltrim.json
...
//      - $XDG_CONFIG_HOME/signaltrim/config.json (any platform, if set)
//      - ~/.config/signaltrim/config.json (macOS / Linux fallback)
//      - %APPDATA%\signaltrim\config.json (Windows fallback)
//   4. 'full'
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'signaltrim');
  }
  ...
  return path.join(os.homedir(), '.config', 'signaltrim');
}
...
// Walk up from `start` looking for a repo-local signaltrim config. Returns the
// absolute path of the first match, or null. Stops at the filesystem root.
// Candidates per dir (first wins): .signaltrim/config.json, .signaltrim.json.
//
// Bounded to 64 levels to defend against symlink cycles on pathological mounts.
function findRepoConfigPath(start) {
  try {
    let dir = path.resolve(start || process.cwd());
    const candidates = ['.signaltrim/config.json', '.signaltrim.json'];
    for (let i = 0; i < 64; i++) {
...
function getDefaultMode() {
  ...
  const envMode = process.env.SIGNALTRIM_DEFAULT_MODE;
...
// flag path (~/.claude/.signaltrim-active) with a symlink to clobber other files.
//
...
//
// Set SIGNALTRIM_DEBUG=1 to emit stderr diagnostics when flag writes are refused.
//
// Silent-fails on any filesystem error — the flag is best-effort.
function safeWriteFlag(flagPath, content) {
  const debug = process.env.SIGNALTRIM_DEBUG === '1';
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (!realStat.isDirectory()) {
          if (debug) process.stderr.write(`[signaltrim] safeWriteFlag: symlink target ${realFlagDir} is not a directory\n`);
          return;
        }
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[signaltrim] safeWriteFlag: symlink target ${realFlagDir} owned by uid ${realStat.uid}, not current user ${process.getuid()}\n`);
            return;
          }
        } else {
          ...
          if (!normalizedReal.toLowerCase().startsWith(normalizedHome.toLowerCase() + path.sep) &&
            ...
            if (debug) process.stderr.write(`[signaltrim] safeWriteFlag: symlink target ${normalizedReal} is outside home directory ${normalizedHome}\n`);
            return;
          }
        }
      } else {
    ...
    const tempPath = path.join(realFlagDir, `.signaltrim-active.${process.pid}.${Date.now()}`);
...
// ($CLAUDE_CONFIG_DIR/.signaltrim-history.jsonl).
//
  ...
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[signaltrim] appendFlag: symlink target ${realDir} owned by uid ${realStat.uid}\n`);
            return;
          }
        } else {
          const home = os.homedir();
...
// append {ts, mode, prev} to $CLAUDE_CONFIG_DIR/.signaltrim-mode-log.jsonl so
// signaltrim-stats can attribute output tokens to the mode that was active when
// each message was generated, instead of whatever mode the flag holds at
// stats time. mode/prev are a VALID_MODES string or null (null = signaltrim off).
...
const MODE_LOG_BASENAME = '.signaltrim-mode-log.jsonl';

function recordModeChange(claudeDir, newMode) {
  try {
    const current = readFlag(path.join(claudeDir, '.signaltrim-active'));
