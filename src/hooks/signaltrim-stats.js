#!/usr/bin/env node
// signaltrim-stats — read the active Claude Code session log, print real token
// usage plus an estimated savings figure from the benchmark in benchmarks/.
//
// Run directly:    node hooks/signaltrim-stats.js
// Inside Claude:   /signaltrim-stats triggers this via the UserPromptSubmit hook.
...
const os = require('os');
const { readFlag, appendFlag, readHistory, safeWriteFlag, VALID_MODES, MODE_LOG_BASENAME } = require('./signaltrim-config');
...
// Detect *.original.md / *.md pairs left behind by signaltrim-compress. The
...
// estimate (verbose tokens counted as compressed) or zero it (signaltrim tokens
// counted as uncompressed). The mode tracker + SessionStart hook append
// {ts, mode, prev} rows to .signaltrim-mode-log.jsonl on every actual transition;
...
// a mode string or 'none' (signaltrim inactive).
...
// would-be OUTPUT tokens that signaltrim avoided. That is the only ratio we can
...
// Pro/Max limits, and are not reduced by signaltrim, so real limit relief is far
...
function formatHistory({ sessions, outputTokens, estSavedTokens, estSavedUsd, since }) {
  ...
  if (sessions === 0) {
    return `\nSignalTrim Stats — Lifetime${window}\n${sep}\nNo sessions logged yet — run /signaltrim-stats inside any session to start tracking.\n${sep}\n`;
  }
...
function formatShare({ outputTokens, turns, mode, model, attribution }) {
  if (turns === 0) {
    return 'SignalTrim armed but no turns yet - signaltrim.sh';
  }
  ...
  if (estSavedTokens > 0) {
    ...
    return `SignalTrim saved ${estSavedTokens.toLocaleString()} output tokens${usd} across ${turns} turns this session - signaltrim.sh`;
  }
  return `SignalTrim ${turns} turns, ${outputTokens.toLocaleString()} output tokens this session - signaltrim.sh`;
}
...
function formatStats({ outputTokens, cacheReadTokens, turns, mode, model, sessionPath, compressed, attribution }) {
  ...
  if (!uniform) {
    ...
    for (const key of activeKeys) {
      ...
      const label = key === 'none' ? 'signaltrim off' : key;
      const note = r != null
    ...
    // sessions and count against Pro/Max limits) are untouched by signaltrim —
    // any session-usage % would overstate real limit relief. See
    // docs/HONEST-NUMBERS.md.
    footer += ' Reduction is of output tokens only; input/cache usage is unchanged.';
    savings = (`Est. without signaltrim:  ${estNormal.toLocaleString()}\n` +
    ...
    savings = 'SignalTrim not active this session.';
  }
...
function main() {
  ...
  const historyPath = path.join(claudeDir, '.signaltrim-history.jsonl');
  ...
  if (all || sinceArg) {
    ...
    if (sinceArg && sinceMs === null) {
      process.stderr.write(`signaltrim-stats: --since takes Nh or Nd (e.g. 7d, 24h), got: ${sinceArg}\n`);
      process.exit(2);
    }
  ...
  if (!sessionFile) {
    process.stderr.write('signaltrim-stats: no Claude Code session found.\n');
    process.exit(1);
  }
  ...
  // /signaltrim-stats calls in one session emit multiple lines for the same
  ...
  if (parsed.turns > 0) {
    ...
    // cat without parsing JSONL. Updated on every /signaltrim-stats run.
    // Routed through safeWriteFlag — the suffix path is predictable and
    // user-owned, same symlink-clobber surface as the .signaltrim-active flag.
    const agg = aggregateHistory(historyPath, null);
    const suffix = agg.estSavedTokens > 0 ? `saved ${humanizeTokens(agg.estSavedTokens)}` : '';
    safeWriteFlag(path.join(claudeDir, '.signaltrim-statusline-suffix'), suffix);
  }
