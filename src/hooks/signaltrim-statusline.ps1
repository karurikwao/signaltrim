...
$Flag = Join-Path $ClaudeDir ".signaltrim-active"
...
if ([string]::IsNullOrEmpty($Mode) -or $Mode -eq "full") {
    [Console]::Write("${Esc}[38;5;172m[SIGNALTRIM]${Esc}[0m")
} else {
    $Suffix = $Mode.ToUpperInvariant()
    [Console]::Write("${Esc}[38;5;172m[SIGNALTRIM:$Suffix]${Esc}[0m")
}

# Savings suffix: on by default. Opt out via SIGNALTRIM_STATUSLINE_SAVINGS=0.
# Reads a pre-rendered string written by signaltrim-stats.js. Refuses reparse
# points and strips control bytes (matches statusline.sh hardening). Until
# /signaltrim-stats has run at least once, the suffix file is absent and nothing
# is rendered — safe default for fresh installs.
if ($env:SIGNALTRIM_STATUSLINE_SAVINGS -ne "0") {
    $SavingsFile = Join-Path $ClaudeDir ".signaltrim-statusline-suffix"
