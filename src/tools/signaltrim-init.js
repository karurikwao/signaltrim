#!/usr/bin/env node
// signaltrim init — drop the always-on signaltrim activation rule into a target
// repo for every IDE agent we support. Idempotent. Safe to re-run.
//
// Usage:
//   node src/tools/signaltrim-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
//   curl -fsSL https://raw.githubusercontent.com/karurikwao/signaltrim/main/src/tools/signaltrim-init.js | node - [args]
//
...
// existing memory files — that's the job of `/signaltrim:compress`.

const fs = require('fs');
const path = require('path');

// Embedded so the tool works standalone (npx-style) without the src/rules/ dir.
// Mirrors src/rules/signaltrim-activate.md verbatim — keep these in sync.
const RULE_BODY = `SignalTrim compact mode active. Preserve technical substance. Remove filler.
...
Switch level: /signaltrim lite|full|ultra|wenyan
Stop: "stop signaltrim" or "normal mode"

Auto-Clarity: drop signaltrim for security warnings, irreversible actions, user confused. Resume after.

Boundaries: code/commits/PRs written normal.
`;

const SENTINEL = 'SignalTrim compact mode active';
...
// lives at bin/lib/openclaw.js; we require it lazily so signaltrim-init.js
...
const AGENTS = [
  { id: 'cursor',   file: '.cursor/rules/signaltrim.mdc',
    frontmatter: '---\ndescription: "SignalTrim mode — terse communication, 65% fewer output tokens (measured), full technical accuracy"\nalwaysApply: true\n---\n\n',
    mode: 'replace' },
  { id: 'windsurf', file: '.windsurf/rules/signaltrim.md',
    frontmatter: '---\ntrigger: always_on\n---\n\n',
    mode: 'replace' },
  { id: 'cline',    file: '.clinerules/signaltrim.md',
    frontmatter: '',
    mode: 'replace' },
  ...
  { id: 'openclaw', description: '~/.openclaw/workspace/{skills/signaltrim/, SOUL.md}',
    installer: 'openclaw' },
];
...
function loadRuleBody() {
  ...
  try {
    const local = path.join(__dirname, '..', 'rules', 'signaltrim-activate.md');
...
function processOpenclaw(opts) {
  ...
  if (!helper) {
    return {
      ...
      label: 'x',
      detail: '~/.openclaw/workspace (helper unavailable in standalone curl|node mode — use `npx -y github:karurikwao/signaltrim -- --only openclaw`)',
    };
  }
...
function help() {
  console.log(`signaltrim init — drop always-on signaltrim rule into a target repo

Usage: signaltrim-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
...
function main() {
  ...
  console.log(`SignalTrim init — ${opts.target}${opts.dryRun ? ' (dry run)' : ''}\n`);
