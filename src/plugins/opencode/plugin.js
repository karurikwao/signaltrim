...
//
// Provides dynamic signaltrim mode tracking for opencode:
// - Writes the mode flag on each session start (via the `event` dispatcher)
// - Parses user messages for /signaltrim commands and natural-language toggles
// - Injects per-turn reinforcement into the system prompt
//
// Bun ESM module; loads the existing security-hardened helpers from
// signaltrim-config.js via createRequire so the symlink-safe flag-write code
// lives in one place.
//
...
//   ├── package.json
//   ├── plugin.js              ← this file
//   └── signaltrim-config.cjs     ← copied sibling of src/hooks/signaltrim-config.js
//
// The always-on signaltrim ruleset is provided separately via
...
// When installed: signaltrim-config.cjs sits next to plugin.js (copied by
...
// src/hooks/signaltrim-config.js, which lives in a directory whose own
...
// signaltrim-config needs (fs/path/os).
function loadConfig() {
  const installed = join(here, 'signaltrim-config.cjs');
  const dev = join(here, '..', '..', 'hooks', 'signaltrim-config.js');
...
// Modes handled by independent skills — not selectable via /signaltrim <arg>.
...
const flagPath = path.join(opencodeConfigDir(), '.signaltrim-active');

function reinforcementLine(mode) {
  return 'SIGNALTRIM MODE ACTIVE (' + mode + '). ' +
...
// null when the prompt doesn't change state. Mirrors signaltrim-mode-tracker.js.
function parseModeChange(promptRaw) {
  ...
  // literal quote characters ("/signaltrim ultra"\n) — unwrap symmetric quotes
  ...
  // Natural-language deactivation — checked before activation so a stop request
  // doesn't trip the activation regex.
  if (/\b(stop|disable|deactivate|turn off)\b.*\bsignaltrim\b/i.test(prompt) ||
      /\bsignaltrim\b.*\b(stop|disable|deactivate|turn off)\b/i.test(prompt) ||
      /\bnormal mode\b/i.test(prompt)) {
    return 'off';
  }

  // Expanded /signaltrim command template. opencode replaces a typed
  // "/signaltrim <level>" with the command file's body ("Activate signaltrim
  ...
  const tpl = /^activate signaltrim mode:[ \t]*(\S*)/.exec(prompt);
  if (tpl) {
    const arg = tpl[1] || '';
  ...
  if (/\b(activate|enable|turn on|start|use)\b.*\bsignaltrim\b/i.test(prompt) ||
      /\bsignaltrim\b.*\b(mode|activate|enable|turn on|start)\b/i.test(prompt)) {
  ...
  if (prompt.startsWith('/signaltrim')) {
    ...
    if (cmd === '/signaltrim-commit')   return 'commit';
    if (cmd === '/signaltrim-review')   return 'review';
    if (cmd === '/signaltrim-compress') return 'compress';
...
export const SignalTrimPlugin = async (_ctx) => {
  ...
  // Intercept user messages to detect /signaltrim commands and natural-language
  ...
  // Inject the reinforcement line into the system prompt when signaltrim is
...
export default SignalTrimPlugin;
