#!/usr/bin/env node
// signaltrim-shrink — MCP middleware that proxies an upstream MCP server and
// compresses prose fields so the model sees fewer tokens.
//
// Usage:
//   signaltrim-shrink <upstream-command> [...args]
//
// Example wrapping the filesystem MCP server:
//   "mcpServers": {
//     "fs-shrunk": {
//       "command": "npx",
//       "args": ["signaltrim-shrink", "npx", "@modelcontextprotocol/server-filesystem", "/some/path"]
//     }
//   }
//
// Compression is applied to:
//   - "description" fields in tools/list, prompts/list, resources/list responses
//   - same boundaries as signaltrim-compress: code, URLs, paths, identifiers preserved
//
...
//   SIGNALTRIM_SHRINK_FIELDS   comma-separated extra field names to compress
//                           (default: description)
//   SIGNALTRIM_SHRINK_DEBUG=1  log compression deltas to stderr
...
if (args.length === 0) {
  process.stderr.write('signaltrim-shrink: missing upstream command.\n');
  process.stderr.write('Usage: signaltrim-shrink <upstream-command> [...args]\n');
  process.exit(2);
}

const debug = process.env.SIGNALTRIM_SHRINK_DEBUG === '1';
const fields = (process.env.SIGNALTRIM_SHRINK_FIELDS || 'description')
  .split(',').map(s => s.trim()).filter(Boolean);

const { getSpawnOptions, assertSafeSpawnInput } = require('./spawn-options');

try {
  assertSafeSpawnInput(args[0], args.slice(1));
} catch (err) {
  process.stderr.write(`signaltrim-shrink: refusing unsafe upstream command: ${err.message}\n`);
  process.exit(2);
}
...
upstream.on('error', err => {
  process.stderr.write(`signaltrim-shrink: failed to spawn upstream: ${err.message}\n`);
  process.exit(1);
});
            ...
            if (out !== before) {
              ...
              if (debug) {
                process.stderr.write(
                  `[signaltrim-shrink] ${arrayName}.${item.name || '?'}.${field}: ` +
